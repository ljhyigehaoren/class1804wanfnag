...
from task import set_wanfangtask
class WflunwenSpider(scrapy.Spider):
    name = 'wflunwen'
    allowed_domains = ['wanfangdata.com.cn']
    start_urls = set_wanfangtask()
    # start_urls = [
    #     #政治
    ...
    def parse(self, response):
        ...
        if 'NSTR' in response.url:
            #http://s.wanfangdata.com.cn/NSTR.aspx?q=%e6%94%bf%e6%b2%bb&f=top&p=2
            pattern = re.compile('.*?q=(.*?)&f.*?p=(\d+)', re.S)
            result = re.findall(pattern, response.url)[0]
            print(result)
            keyword = parse.unquote(result[0])
            tag = 'KJ'
            currentPage = result[1]
        elif 'Paper' in response.url:
        ...
        if len(itemList) > 0:
            for item in itemList:
                ...
                if tag == 'HY':
                    ...
                    #http://www.wanfangdata.com.cn/details/detail.do?_type=tech&id=1048
                    newItemUrl = 'http://www.wanfangdata.com.cn/details/detail.do?_type=tech&id=' + itemId
                    info = {
                        'searchKeyWord': keyword,
                        'searchType': 'tech',
                    }
                ...
                yield scrapy.Request(
                    ...
                    meta={'info': info, 'title': itemTitle}
                )
